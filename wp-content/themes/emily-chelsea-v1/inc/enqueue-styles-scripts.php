...
function ttg_wp_scripts()
{
    ...
    if (
        ...
        || is_page_template('page-ring-setting.php')
        || is_page_template('page-stone-first.php')
    ) {
    ...
    if (TTG_Build_Ring::is_ring_page() || TTG_Build_Ring::is_stone_page()) {
        wp_enqueue_style('build-ring-step', get_template_directory_uri() . '/src/dist/css/components/build-ring-step.css');
        wp_enqueue_script('build-ring-step', get_template_directory_uri() . '/src/dist/js/components/build-ring.js', [], false, true);
    }
