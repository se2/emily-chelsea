...
if (TTG_Build_Ring::is_ring_page() && is_singular('product')) {
	global $post;
	$breadcrumb = [
		array('Rings', TTG_Build_Ring::get_ring_page()),
		array($post->post_title, ''),
	];
} else if (TTG_Build_Ring::is_stone_page() && is_singular('product')) {
	global $post;
	$breadcrumb = [
		array('Center stones', TTG_Build_Ring::get_stone_page()),
		array($post->post_title, ''),
	];
