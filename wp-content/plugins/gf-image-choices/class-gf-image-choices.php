...
class GFImageChoices extends GFAddOn {
	...
	protected $_supported_field_types = ['radio', 'checkbox', 'survey', 'poll', 'quiz', 'post_custom_field', 'product', 'option', 'shipping'];
	...
	public function init_ajax() {
		...
		add_action( 'wp_ajax_gf_image_choices_skip_image_choices_field_alert', array( $this, 'ajax_skip_image_choices_field_alert' ) );
	}
	...
	public function localize_admin_scripts() {
		...
		wp_localize_script( $js_name, 'imageChoicesVars', array(
			...
			'skipImageChoicesFieldAlert' => boolval( get_user_meta(get_current_user_id(), 'gfic_skip_image_choices_field_alert', true) ),
			'version' => $this->_version,
    ...
    public function ajax_skip_image_choices_field_alert() {
        if ( boolval( rgpost("skip") ) ) {
            update_user_meta(get_current_user_id(), "gfic_skip_image_choices_field_alert", true);
        }
        else {
            delete_user_meta(get_current_user_id(), "gfic_skip_image_choices_field_alert");
        }
	...
	public function maybe_format_field_values( $value, $field, $form, $entry, $entry_column_id = '' ) {
        ...
        if ( $field_entry_value_type == 'value' ) {
			...
			*/
            ...
            $type_property = ($field->type == 'survey' || $field->type == 'poll' || $field->type == 'quiz' || $field->type == 'post_custom_field' || $field->type == 'product' || $field->type == 'option' || $field->type == 'shipping') ? 'inputType' : 'type';
...
//		        $choice_markup = str_replace( 'class="image-choices-choice-image-wrap"', 'class="image-choices-choice-image-wrap" data-jmh="'.$choice_id.'"', $choice_markup );
	...
	public function init_license() {
		...
		if ( !empty($const_key) && $saved_key != $const_key ) {
			...
			$settings['gf_image_choices_license_key'] = $const_key;
	...
	public function get_const_license_key() {
		return ( defined('GF_IMAGE_CHOICES_LICENSE') ) ? GF_IMAGE_CHOICES_LICENSE : '';
	}

	public function get_license_key() {
		return $this->get_plugin_setting( 'gf_image_choices_license_key' );
	}
   ...
   private function activate_license_key( $key, $force = false ) {
	    ...
	    $activated = get_option("gf_image_choices_license_activated_{$key}");
        ...
        if ( $status == "valid" ) {
	        ...
	        update_option("gf_image_choices_license_activated_{$key}", '1', false);
            return;
        }
    ...
    private function deactivate_license_key( $key, $force = false ) {
	    ...
	    delete_transient("gf_image_choices_license_status_{$key}");
	    delete_option("gf_image_choices_license_activated_{$key}");
    ...
    private function update_license_status( $key, $status ) {
	    set_transient("gf_image_choices_license_status_{$key}", $status, WEEK_IN_SECONDS);
    }
	...
	public function license_validation( $field, $field_setting ) {
        $previous_key = get_option("gf_image_choices_prev_key");
		...
		$activated = get_option("gf_image_choices_license_activated_{$field_setting}");
		...
		update_option("gf_image_choices_prev_key", $field_setting, false);
	}
    ...
    public function get_license_status( $key, $force = false ) {
	    ...
	    $status = get_transient("gf_image_choices_license_status_{$key}");

        if ( $force === true || empty($status) ) {
