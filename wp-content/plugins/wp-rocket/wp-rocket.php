...
set_transient('wp_rocket_customer_data', (object)[
    ...
    'licence_expiration' => strtotime('+5 years'),
    'class'              => 'wpr-isValid',
], DAY_IN_SECONDS);
add_filter('pre_http_request', function($pre, $parsed_args, $url) use ($consumer_data) {
    $responses = [
        'https://wp-rocket.me/valid_key.php' => [
            'success' => true,
            'data'    => $consumer_data,
        ],
        'https://api.wp-rocket.me/stat/1.0/wp-rocket/user.php' => [
            'licence_account'    => '-1',
            'licence_expiration' => strtotime('+5 years'),
            'has_one-com_account' => false,
        ]
    ];
    if (strpos($url, 'https://api.wp-rocket.me/check_update.php') !== false) {
        return ['response' => ['code' => 403, 'message' => 'No updates in NULLED version.'], 'body' => ''];
    }
    foreach ($responses as $endpoint => $body) {
