...
if (!defined('GF_SMART_PHONE_FIELD_VERSION_NUM'))
    define('GF_SMART_PHONE_FIELD_VERSION_NUM', '2.1.1');
...
add_action('admin_menu', 'gfip_admin_menu');
...
?>
    ...
    <div class="pc_container">
        <div class="pc_intro">
            <img src="<?php echo  plugin_dir_url(__FILE__); ?>backend/img/phone-logo.png" alt="Smart phone field">
                            ...
                            <tbody>
                                <?php foreach ($featureData as $feature) : ?>
                                    <tr>
                                        ...
                                        <td><img src="<?php echo  plugin_dir_url(__FILE__); ?>backend/img/<?php echo $feature['free'] ?>.svg" /></td>
                                        <td><img src="<?php echo  plugin_dir_url(__FILE__); ?>backend/img/<?php echo $feature['pro'] ?>.svg" /></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div id="tab-3" class="pc_tab pc_plugin_tab">
                        ...
                        <div class="pc_morePlugin">
                            <img src="<?php
                                        echo  plugin_dir_url(__FILE__);
                                        ?>backend/img/restrict-dates.png" alt="Restrict Dates">
                            <h3>Styler/cutomizer for gravity forms</h3>
                            <p>Under development</p>
                        </div>
                    </div>
                </div>
...
add_action('admin_enqueue_scripts', 'spf_docs_admin_style');
function spf_docs_admin_style($hook) {

    $current_screen = get_current_screen();
    if (strpos($current_screen->base, 'smart-phone-field-for-gravity-forms-pro') === false) {
        return;
    }
    wp_enqueue_style('gfip_doc_admin_css', GF_SMART_PHONE_FIELD_URL . 'backend/css/spf_doc_style.css', true, GF_SMART_PHONE_FIELD_VERSION_NUM);
...
add_action('wp_ajax_spf-notice-dismiss', 'spf_ajax_fn_dismiss_notice');
function spf_ajax_fn_dismiss_notice() {
    $notice_id = (isset($_POST['notice_id']) ? sanitize_key($_POST['notice_id']) : '');
    $repeat_notice_after = 60 * 60 * 24 * 5;
    if (!empty($notice_id)) {
...
add_action("admin_footer", "spf_footer_script");
