...
use FSProVendor\Symfony\Component\Mailer\MailerInterface;
use FSProVendor\Symfony\Component\Mailer\Transport\TransportInterface;
use FSProVendor\Symfony\Component\Mime\Email;
/**
 ...
 */
class SymfonyMailerHandler extends \FSProVendor\Monolog\Handler\MailHandler
{
     ...
     */
    public function __construct($mailer, $email, $level = \FSProVendor\Monolog\Logger::ERROR, bool $bubble = \true)
    {
     ...
     */
    protected function buildMessage(string $content, array $records) : \FSProVendor\Symfony\Component\Mime\Email
    {
        $message = null;
        if ($this->emailTemplate instanceof \FSProVendor\Symfony\Component\Mime\Email) {
            $message = clone $this->emailTemplate;
        } elseif (\is_callable($this->emailTemplate)) {
            $message = ($this->emailTemplate)($content, $records);
        }
        if (!$message instanceof \FSProVendor\Symfony\Component\Mime\Email) {
            $record = \reset($records);
            throw new \InvalidArgumentException('Could not resolve message as instance of Email or a callable returning it' . ($record ? \FSProVendor\Monolog\Utils::getRecordMessageForException($record) : ''));
        }
        if ($records) {
