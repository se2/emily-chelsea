...
use FSProVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use FSProVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \FSProVendor\Monolog\Handler\Handler implements \FSProVendor\Monolog\Handler\ProcessableHandlerInterface, \FSProVendor\Monolog\ResettableInterface, \FSProVendor\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \FSProVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\FSProVendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \FSProVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \FSProVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \FSProVendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \FSProVendor\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \FSProVendor\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \FSProVendor\Monolog\Handler\HandlerInterface) {
    ...
    public function setFormatter(\FSProVendor\Monolog\Formatter\FormatterInterface $formatter) : \FSProVendor\Monolog\Handler\HandlerInterface
    {
        ...
        if ($handler instanceof \FSProVendor\Monolog\Handler\FormattableHandlerInterface) {
