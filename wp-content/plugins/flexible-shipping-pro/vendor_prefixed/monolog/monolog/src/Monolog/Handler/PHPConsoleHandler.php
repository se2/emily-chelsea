...
use FSProVendor\PhpConsole\Connector;
use FSProVendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use FSProVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FSProVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\FSProVendor\PhpConsole\Connector $connector = null, $level = \FSProVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('FSProVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\FSProVendor\PhpConsole\Connector $connector = null) : \FSProVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FSProVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FSProVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FSProVendor\PhpConsole\Helper::isRegistered()) {
            \FSProVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FSProVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \FSProVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \FSProVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FSProVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FSProVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
