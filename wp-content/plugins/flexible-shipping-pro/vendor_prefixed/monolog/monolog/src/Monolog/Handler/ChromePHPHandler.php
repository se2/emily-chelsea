...
use FSProVendor\Monolog\Formatter\ChromePHPFormatter;
 ...
 */
class ChromePHPHandler extends \FSProVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \FSProVendor\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \FSProVendor\Monolog\Utils::jsonEncode(self::$json, \FSProVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \FSProVendor\Monolog\Logger::WARNING, 'level_name' => \FSProVendor\Monolog\Logger::getLevelName(\FSProVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
