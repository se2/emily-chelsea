 ...
 */
class PushoverHandler extends \FSProVendor\Monolog\Handler\SocketHandler
{
    /** @var string */
    private $token;
     ...
     */
    public function __construct(string $token, $users, ?string $title = null, $level = \FSProVendor\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useSSL = \true, $highPriorityLevel = \FSProVendor\Monolog\Logger::CRITICAL, $emergencyLevel = \FSProVendor\Monolog\Logger::EMERGENCY, int $retry = 30, int $expire = 25200, bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        ...
        $this->highPriorityLevel = \FSProVendor\Monolog\Logger::toMonologLevel($highPriorityLevel);
        $this->emergencyLevel = \FSProVendor\Monolog\Logger::toMonologLevel($emergencyLevel);
        $this->retry = $retry;
        $this->expire = $expire;
    }
    ...
    private function buildContent(array $record) : string
    {
        ...
        $message = \FSProVendor\Monolog\Utils::substr($message, 0, $maxMessageLength);
    ...
    {
        $this->highPriorityLevel = \FSProVendor\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
     * @param int|string $value
    ...
    {
        $this->emergencyLevel = \FSProVendor\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
