...
class PluginUpgrade
{
    ...
    private function react_to_wp_error_response(\WP_Error $remote_response) : \stdClass
    {
        ...
        $response->message = \wp_kses_post('<span style="color: red" class="error">' . \sprintf(\__('Error while connecting to remote server %s. Please contact your hosting provider or try again later. Errors:  ', 'flexible-shipping-pro'), $this->server) . \implode(', ', $remote_response->get_error_messages()) . '</span>');
        return $response;
    }
    /**
    ...
    private function react_to_valid_remote_response(array $parsed_response) : \stdClass
    {
        ...
        if (empty($parsed_response['package'])) {
            (new \FSProVendor\WPDesk\License\LicenseServer\PluginLicense($this->plugin_info))->set_inactive();
        } else {
            (new \FSProVendor\WPDesk\License\LicenseServer\PluginLicense($this->plugin_info))->set_active();
        }
        return $response;
    }
    private function render_changelog(string $plugin_version, string $changelog) : string
    {
        $parser = new \FSProVendor\WPDesk\License\Changelog\Parser($changelog);
        $parser->parse();
        $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
        $changes = new \FSProVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
        if (\iterator_count($changes) > 0) {
            $changelogFormatter = new \FSProVendor\WPDesk\License\Changelog\Formatter($changes);
    ...
    private function react_to_nonsense_response($remote_response) : \stdClass
    {
        ...
        if (isset($remote_response['response']['code'])) {
            $message = \sprintf(\__('Error while connecting to remote server %s. Please contact with your hosting provider. Cannot parse response. Response code: %s Message: %s', 'flexible-shipping-pro'), $this->server, $remote_response['response']['code'], $remote_response['body']);
        } else {
            $message = \sprintf(\__('Error while connecting to remote server %s. Please contact with your hosting provider. Cannot parse response: %s', 'flexible-shipping-pro'), $this->server, \json_encode($remote_response));
        }
