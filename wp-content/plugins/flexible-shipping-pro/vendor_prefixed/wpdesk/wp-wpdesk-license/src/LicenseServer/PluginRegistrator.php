...
class PluginRegistrator implements \FSProVendor\WPDesk\License\PluginRegistratorInterface
{
    ...
    {
        (new \FSProVendor\WPDesk\License\LicenseServer\PluginUpgrade($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new \FSProVendor\WPDesk\License\LicenseServer\PluginExternalBlocking($this->plugin_info, $this->server, self::get_token()))->hooks();
